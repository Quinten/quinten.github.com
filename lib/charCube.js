...
let create = () => {
    ...
    let spreadX = (...args) => {
        let spreadIndex = 0;
        for (let c = 0; c < nCubes; c++ ) {
            cube[c].extra.xhome = (xOffset - (args[0][spreadIndex][0] * gridSize)) * args[1];
            spreadIndex = (spreadIndex < (args[0].length - 1)) ? (spreadIndex + 1) : 0;
        }
        rotationStep();
    }
    ...
    let spreadY = (...args) => {
        let spreadIndex = 0;
        for (let c = 0; c < nCubes; c++ ) {
            cube[c].extra.yhome = yOffset - (args[0][spreadIndex][1] * gridSize);
            spreadIndex = (spreadIndex < (args[0].length - 1)) ? (spreadIndex + 1) : 0;
        }
        rotationStep();
    }
    ...
    let spreadZ = (...args) => {
        let spreadIndex = 0;
        for (let c = 0; c < nCubes; c++ ) {
            cube[c].extra.zhome = (zOffset - (args[0][spreadIndex][0] * gridSize)) * args[1];
            spreadIndex = (spreadIndex < (args[0].length - 1)) ? (spreadIndex + 1) : 0;
        }
        rotationStep();
    }
